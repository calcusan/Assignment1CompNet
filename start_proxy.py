...
def parse_virtual_hosts(config_file):
    ...
    for host, block in host_blocks:
        ...
        proxy_set_headers = {}
        for header, value in re.findall(r'proxy_set_header\s+([^\s]+)\s+([^;]+);', block):
            proxy_set_headers[header.strip()] = value.strip()
        ...
        routes[normalized_host] = {
            'proxy_pass': proxy_passes,
            'dist_policy': dist_policy,
            'proxy_set_header': proxy_set_headers,
        }
